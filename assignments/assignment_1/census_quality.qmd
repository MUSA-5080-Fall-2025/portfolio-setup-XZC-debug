...
Number of counties: [YOUR ANSWER]
...
[Write what these results imply for algorithmic decisions. E.g., "Counties with higher MOE% may be mis-ranked if we sort by income alone, which could misdirect funding."]
```{r}
# Neighborhood-Level Analysis
  # Select Focus Counties
focus_counties <- c("Philadelphia", "Allegheny", "SomeCounty")  # <-- change these
...
```
Short commentary (2–3 sentences):
[Describe any pattern you see. For example, "Smaller-population tracts tend to have higher MOE."]
```{r}
...
Immediate algorithm use (High): [YOUR ANSWER]

Needs monitoring (Moderate): [YOUR ANSWER]

Alt approach / manual review (Low): [YOUR ANSWER]
